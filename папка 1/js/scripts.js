...
function initNullStep () {
	...
	$('.type_3_btn').click(function () {
		...
		card.logo = 'img/logo_3.png'; 
		card.strip = 'img/strip_3.png';
...
function initSecondStep () {
	if (($(".name_company").val()==="") && ($(".design_li.menu_li").hasClass("active"))){
...
$(document).ready(function () {
	...
	$(".send.address_search_btn.send_phone").on("click", function(event){
		event.preventDefault();
		if ($(".phone").val()!==""){
			sweetAlert("Успешно","На указанный телефон была отправлена ссылка на Вашу карту", "success");
	...
	$(".delcur").on("click", function(event){
		...
		$(".notifications_icon_wrapper.bind-notifications-icon img").attr("src","");
		$(this).fadeOut();
	});	
...
if (item.val()!==""){
	...
	if(!pattern.test(item.val())){	
       ...
       sweetAlert("Успешно","На указанный e-mail была отправлена ссылка на Вашу карту", "success");
    }
}
}
